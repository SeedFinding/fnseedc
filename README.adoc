...
* https://docs.google.com/document/d/1stTJAjLmCXtqctdFOpuv4lylegcmfO8mFrptFtwqb78/edit[Solid Talk on reversal of Math.random (in-game, infact) and how it is done , Context: Mess Detection by EarthComputer - 10/10 the best one to clear up any mental clog related to the concept] ☆☆☆☆☆
...
_The NERD documents are long and are only recommended if you want to start from the basics, especially the William Stein notes are by far better to start with.Pick any one of the top 4 modular arithmetic notes if confused. Then for the follow up read the modular equivalences article._


.Hensel Lifting (Use cases -> (Discrete log problem : find number of calls between two seeds) and (Mod Inversion : Find inverses speedily)
...
* https://github.com/KaptainWutax/MineMap[MineMap by KaptainWutax - amidst but really better <lol>]
